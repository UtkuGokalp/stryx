...
void MeasureSensorPulse(TIM_HandleTypeDef* htim)
{
	UNUSED(htim);
...
void StartDistanceMeasurement(TIM_HandleTypeDef* htim) //Called every ~60 ms
{
	UNUSED(htim);
	//Send a 10 us pulse
...
{
	HAL_TIM_RegisterCallback(&htim2, HAL_TIM_IC_CAPTURE_CB_ID, MeasureSensorPulse);
	HAL_TIM_RegisterCallback(&htim6, HAL_TIM_PERIOD_ELAPSED_CB_ID, StartDistanceMeasurement);
